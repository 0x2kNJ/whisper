...
import { UNISWAP_API_BASE, getEnvOrThrow } from './config'
...
async function getOnChainQuote(params: QuoteParams): Promise<QuoteResponse> {
  ...
  try {
    ...
    if (isToken0In) {
      ...
      amountOut = (amountIn * sqrtPriceX96 * sqrtPriceX96) / (BigInt(1) << BigInt(192))
    } else {
      // Selling token1 (WETH), getting token0 (USDC)
      // amountOut = amountIn * 2^192 / sqrtPriceX96^2
      amountOut = (amountIn * (BigInt(1) << BigInt(192))) / (sqrtPriceX96 * sqrtPriceX96)
    }

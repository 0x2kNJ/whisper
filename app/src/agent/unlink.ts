...
async function pollUntilRelayed(
  ...
  while (Date.now() - start < timeoutMs) {
    ...
    if (TERMINAL.has(result.status)) {
      ...
      // Try to extract on-chain tx hash from the result
      const r = result as unknown as Record<string, unknown>
      const onChainHash = (r.txHash || r.tx_hash || r.transactionHash || r.hash || r.onChainTxHash) as string | undefined
      return { txId, onChainHash: onChainHash?.startsWith('0x') ? onChainHash : undefined }
    }
    break
  }
...
): Promise<{ txHash: string; onChainHash?: string }> {
  try {
    ...
    const poll = await pollUntilRelayed(client.sdk, result.txId)

    return { txHash: poll.onChainHash || result.txId, onChainHash: poll.onChainHash }
  } catch (err) {
    throw new Error(
...
export async function transfer(
  ...
  try {
    ...
    const poll = await pollUntilRelayed(client.sdk, result.txId)
    return { txHash: poll.onChainHash || result.txId, status: 'relayed' }
  } catch (err) {
    throw new Error(
...
export async function batchTransfer(
  ...
  try {
    ...
    const poll = await pollUntilRelayed(client.sdk, result.txId)
    return { txHash: poll.onChainHash || result.txId }
  } catch (err) {
    throw new Error(
...
export async function execute(
  ...
  try {
    ...
    const poll = await pollUntilRelayed(client.sdk, tx_id)
    return { txHash: poll.onChainHash || tx_id }
  } catch (err) {
    throw new Error(
